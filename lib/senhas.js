...
// Procura a conta pelo e-mail.
//
// ATENCAO, e' uma pegadinha cara: a listagem de usuarios da Supabase aceita
// SO "page" e "per_page". Nao existe filtro por e-mail. A primeira versao
// disto mandava "&email=..." e recebia de volta a primeira pagina inteira,
// ignorando o filtro -- com per_page=1 isso devolvia UM usuario qualquer e a
// recuperacao de senha falhava para todo mundo.
//
// Este caminho e' so o plano B. O certo e' guardar o id da Supabase no nosso
// banco na hora de criar a conta (clientes.supabase_id) e nao procurar nada.
export async function acharPorEmail(email) {
  const alvo = String(email || '').toLowerCase()
  for (let pagina = 1; pagina <= 10; pagina += 1) {
    const r = await chamar('/auth/v1/admin/users?page=' + pagina + '&per_page=1000', {
      metodo: 'GET',
      chave: segredo(),
    })
