...
// A operadora pode avisar da mesma compra mais de uma vez. O stripe_session_id
// é único no banco, então a segunda vez não vira compra nova — e "INSERT OR
// IGNORE" evita que isso vire erro. Compras do Mercado Pago entram na mesma
// coluna com o prefixo mp_.
//
// Devolve true só quando a compra entrou agora, para quem chama saber se é a
// primeira vez que ouve falar dela.
export async function registrarCompra({ clienteId, sessionId, paymentIntent, valor, moeda, forma }) {
  const db = getDB()
  if (!db) return false
  const r = await executar(
    db,
    `INSERT OR IGNORE INTO compras
       (cliente_id, stripe_session_id, stripe_payment_intent, valor_centavos, moeda, forma_pagamento)
     VALUES (?, ?, ?, ?, ?, ?)`,
    clienteId,
    sessionId,
    paymentIntent || null,
