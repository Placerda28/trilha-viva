...
import { formaDoMP } from '@/lib/gestao/forma'
...
export async function criarPreferencia({ email, nome, referencia, origem, titulo, descricao, valor, expiraEmMin, rastreio = {} }) {
  // O Mercado Pago quer a data com fuso (ex.: 2026-09-24T15:30:00.000-03:00).
  const validade = expiraEmMin
    ? {
        expires: true,
        expiration_date_to: new Date(Date.now() + expiraEmMin * 60000 - 3 * 3600000)
          .toISOString()
          .replace('Z', '-03:00'),
      }
    : {}
  const pref = await chamar('/checkout/preferences', {
    ...
    corpo: {
      ...
      metadata: { email, nome, referencia, ...rastreio },
