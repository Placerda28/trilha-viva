...
export function montarCorpoPreferencia({
  ...
  // venceEmMs (instante exato) tem preferência sobre expiraEmMin: é o que o
  // checkout com cupom usa para o link nunca passar da validade do cupom.
  const instante = venceEmMs || (expiraEmMin ? agora + expiraEmMin * 60000 : null)
  const venceEm = instante ? dataMercadoPago(instante) : null
  // Só a validade da PREFERÊNCIA (até quando dá para começar a pagar). A
  // validade do Pix (date_of_expiration) fica de fora de propósito: o Mercado
  // Pago exige no mínimo 30 minutos a partir de quando o Pix é gerado, e um
  // prazo contado da preferência ficaria abaixo disso e poderia recusar o Pix.
  // A vaga do cupom continua protegida: o aviso "pendente" que chega quando o
  // Pix é gerado estende a reserva (ver estenderReservaCupom).
  const validade = venceEm ? { expires: true, expiration_date_to: venceEm } : {}
  return {
    items: [
      {
