...
// Minutos que o link de pagamento com cupom pode ficar aberto: 30, ou menos se
// o cupom vencer antes disso. Assim ninguém abre o pagamento às 23h50 e paga
// com desconto depois que o cupom venceu.
export function minutosDoLinkComCupom(validoAteIso, agora = Date.now(), maximo = 30) {
  const fim = Date.parse(validoAteIso || '')
  if (!Number.isFinite(fim)) return maximo
  return Math.max(1, Math.min(maximo, Math.floor((fim - agora) / 60000)))
}
...
// Cartão "em análise" pode ser aprovado depois dos 35 minutos da reserva. Se a
// vaga vencesse nesse meio-tempo, outra pessoa poderia ocupá-la e o cupom
// passaria do limite. Enquanto o Mercado Pago disser que o pagamento está
// pendente, a vaga fica guardada por mais 3 horas a cada aviso. Só estende
// reserva que ainda existe (quem já virou compra não tem mais reserva).
export async function estenderReservaCupom(db, referencia, horas = 3) {
