...
export async function adicionarMembro(db, {
  ...
  if (cliente) {
    if (!cliente.supabase_id) {
      await db.prepare('UPDATE clientes SET supabase_id = ? WHERE id = ?')
        .bind(supabaseId, cliente.id)
        .run()
    }
  } else {
    await db.prepare('INSERT INTO clientes (email, supabase_id) VALUES (?, ?)')
      .bind(email, supabaseId)
      .run()
    cliente = await buscarClientePorEmail(db, email)
  }

  const precisaTrocar = contaExistente ? 0 : 1
  await db.prepare(`
    INSERT INTO equipe
    ...
    ON CONFLICT(email) DO UPDATE SET
      ...
      removido_por = NULL,
      removido_em = NULL`)
    .bind(email, criadoPor, precisaTrocar)
    .run()
