...
// As abas da gestão. A Equipe só aparece para o master, e a própria rota dela
// recusa quem não é: esconder aqui é conforto, não segurança.
const ABAS = [
...
export default function GestaoNav({ papel }) {
  const pathname = usePathname()
  const abas = ABAS.filter((aba) => !aba.soMaster || papel === 'master')
  return (
            ...
            <li key={aba.href}>
              <Link
                ...
                className={
                  ...
                  (ativa ? 'border-signal text-ink' : 'border-transparent text-ink-muted hover:text-ink')
                }
              >
                {aba.rotulo}
              </Link>
